...
type AddressReference struct {
	AddressReferenceIdentifier *AddressReferenceIdentifier
...
func CreateAddressReferenceID(id AddressReferenceIdentifier) AddressReference {
	typ := AddressReferenceTypeID
	typStr := AddressReferenceIdentifierTag(typ)
	id.DotTag = typStr

	return AddressReference{
		AddressReferenceIdentifier: &id,
		Type:                       typ,
	}
}
...
func (u *AddressReference) UnmarshalJSON(data []byte) error {
	...
	case "id":
		d = json.NewDecoder(bytes.NewReader(data))
		addressReferenceIdentifier := new(AddressReferenceIdentifier)
		if err := d.Decode(&addressReferenceIdentifier); err != nil {
			return fmt.Errorf("could not unmarshal expected type: %w", err)
		}

		u.AddressReferenceIdentifier = addressReferenceIdentifier
...
func (u AddressReference) MarshalJSON() ([]byte, error) {
	if u.AddressReferenceIdentifier != nil {
		return json.Marshal(u.AddressReferenceIdentifier)
	}
